...
def process_image(path, max_dataset=None, to_pad=None, output_dims=None):
    array = img_to_array(path)
    #array = max_normalize(array)
    #img = nb.load(path)
    #masked_img = threshold_img(img, threshold='50%')
    #array = masked_img.get_fdata()
    # array = mask(array, '/home/leonor/Code/classification_torch/masks/brainmask.nii')
    #array = array / max_dataset
    # array = max_normalize(array)
    array = z_score_normalize(array)


    if to_pad == 'PET':
        array = np.pad(array, ((8, 9), (8, 9), (13, 14)))
    elif to_pad == 'SPECT':
        pass
        #array = np.pad(array, ((2, 3), (1, 2), (2, 3)))
    ...
    #array = resize_array(array, output_dims)
    return array
